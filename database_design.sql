...
SET @neil_geiman_id := (SELECT id FROM authors WHERE author_name = 'Neil Gaiman');

SET @marcel_product_id := (SELECT id FROM authors WHERE author_name = 'Marcel Proust');
...
INSERT INTO quote_topics (quote_id, topic_id) VALUES (1, 2), (1, 3), (2, 2), (2, 3), (3, 1), (3, 2),(4, 1),
       (4, 3), (5, 3), (5, 4), (6, 3), (6, 4);

SET @literature_id := (SELECT id FROM topics WHERE topic = 'literature');

SELECT content, author_name AS author FROM authors a JOIN quoets q ON a.id = q.author_id JOIN
    quote_topics qt on q.id = qt.quote_id WHERE qt.topic_id = @literature_id;